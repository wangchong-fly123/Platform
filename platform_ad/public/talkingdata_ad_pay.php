...
if (isset($_GET['amount']) === true) {
    $amount = $_GET['amount'];
}
if (isset($_GET['currency_type']) === true) {
...
$sth = $dbh->prepare(
    'insert ignore into `tbl_talkingdata_ad_pay`('.
    '`appkey`, `order_id`, `amount`, `currency_type`, `account_id`, `event_time`, `os_version`, '.
    '`device_type`, `idfa`, `mac`, `advertising_id`, '.
    '`android_id`, `imei`, `tdid`, '.
    '`ip`, `spread_url`, `spread_name`, '.
    '`ua`, `click_ip`, `click_time`, `adnet_name`, `appstore_id`, `deeplink`, `channel_package_id`'.
    ') values ('.
    ':appkey, :order_id, :amount, :currency_type, :account_id, :event_time, :os_version, '.
    ':device_type, :idfa, :mac, :advertising_id, '.
    ':android_id, :imei, :tdid, '.
...
$sth->bindValue(':currency_type', $currency_type, PDO::PARAM_STR);
