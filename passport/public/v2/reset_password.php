...
// check message code
if ($auth_service->checkResetPasswordCode(
        $uid, $message_code) === false) {
    $auth_service->addFailedResetPasswordCodeTimes($uid);
...
$result = $account_service->changePassword($account, $password);
if ($result === false) {
    Util::error('change password failed');
} else {
    $auth_service->clearResetPasswordCode($uid);
}
