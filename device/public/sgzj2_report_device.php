...
$sth = $dbh->prepare(
    'select 1 from `tbl_sgzj2_device` '.
...
$sth = $dbh->prepare(
    'replace into `tbl_sgzj2_device`('.
    '`account`, `device_id`,`record_time`,`game_id`,`imei`,`idfa`,`uid` '.
    ') values ('.
    ':account, :device, :record_time, :game_id, :imei, :idfa, :uid)'
);
$sth->bindValue(':account', $account, PDO::PARAM_STR);
$sth->bindValue(':device', $device, PDO::PARAM_STR);
