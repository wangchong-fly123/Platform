    ...
    onSubmitAddPlatform: function() {
        ...
        webix.ajax().sync().get(
            ...
            function(text, data) {
                ...
                webix.alert("추가 실패. 이미 존재하는 플랫폼인지 확인하십시오");
            });
...
var add_platform_form = {
    ...
    elements: [
        { view: "text", name: "id", label: "플랫폼번호(2자리 숫자ID)",
          required: true },
        { view: "text", name: "desc", label: "플랫폼명(플랫폼설명)",
          required: true },
    ...
    rows: [
        { cols: [
            { id: "button:add_platform",
              view: "button", label: "플랫폼 추가 ", width: 100,
              click: event_handler.onAddPlatform
            },
            { id: "button:remove_platform",
              view: "button", label: "풀렛폼 삭제", width: 100,
              hidden: true,
