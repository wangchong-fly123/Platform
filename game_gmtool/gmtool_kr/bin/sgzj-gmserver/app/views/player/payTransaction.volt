...
var event_handler = {
    onSubmitGetPayTransaction: function() {
        var form = $$("form:get_pay_transaction");
        if (form.validate() === false) {
            return;
        }

        webix.ajax().sync().get(
            "{{ url('player/get_pay_transaction') }}",
            form.getValues(),
            function (text, data) {
                var ret = data.json();
                if (!ret) {
...
var get_pay_transaction_form = {
    id: "form:get_pay_transaction",
    view: "form",
    width: 600,
    elements: [
        ...
        { view: "button", label: "조회", width: 100, align: "right",
          click: event_handler.onSubmitGetPayTransaction },
    ],
};
...
var transaction_list = {
    ...
    columns: [
        ...
        { id: "user_id", header: "플랫폼유저ID", width: 300 },
        { id: "transaction_time", header: "거래시간", width: 160 },
        { id: "amount", header: "충전금액" },
        { id: "product_id", header: "상품ID" },
        { id: "product_count", header: "상품수량" },
        { id: "product_name", header: "상품명" },
        { id: "pay_time", header: "결제시간", width: 160 },
        { id: "order_type", header: "결제유형" },
        { id: "channel_number", header: "플랫폼번호" },
        { id: "channel_order_id", header: "플랫폼주문번호", width: 350 },
    ],
