...
var event_handler = {
    onSubmitGetCpsMonthlyReport: function() {
        ...
        webix.ajax().sync().get(
            "{{ url('stat/get_all_cps_monthly_report') }}",
            form.getValues(),
            function (text, data) {
                var ret = data.json();
                if (!ret) {
                ...
                if (ret.all_cps_monthly_report_list) {
                    $$("table:monthly_report").clearAll();
                    $$("table:monthly_report").parse(ret.all_cps_monthly_report_list);
                    $$("table:monthly_report").getFilter("timestamp").value = "";
                    $$("table:monthly_report").getFilter("cps").value = "";
                    webix.alert("조회성공");
...
var get_monthly_report_form = {
    ...
    elements: [
        { view: "button", label: "조회", width: 100, align: "right",
          click: event_handler.onSubmitGetCpsMonthlyReport },
    ],
};
...
var monthly_report_list = {
    ...
    width: 1024,
    columns: [
        { id: "timestamp", header: ["일자", { content: "selectFilter" }],
          width: 100 },
        { id: "cps", header: ["CPS", { content: "selectFilter" }],
          width: 200 },
