...
final class PlayerController extends ControllerBase
{
    ...
    public function accountChangeRoleAction()
    {
        $this->fetchPlayerManagementCache();
    }
	public function playerSilenceAction()
    {
        $this->fetchPlayerManagementCache();
    }
    ...
    public function gmRenameAction()
    {
        $this->fetchPlayerManagementCache();
    }
    ...
    public function getBriefInfoAction()
    {
        ...
        if (isset($ret['brief_info'])) {
            ...
            $brief_info['silence_until_time'] = DataFormatter::timestamp(
                $brief_info['silence_until_time']);
    ...
    public function banAccountAction()
    {
        ...
        if ($ban_hours == 0) {
            $this->opLog("[解封帐号] 服务器: $server_desc 玩家ID: $player_id");
        } else {
            $this->opLog("[封停帐号] 服务器: $server_desc 玩家ID: $player_id ".
                         "封停时间: ${ban_hours}小时");
        }
    ...
    public function changeRoleAccountAction()
    {
        $this->view->disable();
        ...
        if ($uid === null) {
             return $this->error('`uid` is required');
        }
        $old_uid = $this->request->get('old_uid');
        if ($old_uid === null) {
            return $this->error('`old_uid` is required');
        }
        $server_id = $this->request->get('server_id');
        if ($server_id === null) {
            return $this->error('`server_id` is required');
        }
        $old_server_id = $this->request->get('old_server_id');
        if ($old_server_id === null) {
            return $this->error('`old_server_id` is required');
        }
        $src_channel = $this->request->get('src_channel');
        if ($src_channel === null) {
            return $this->error('`src_channel` is required');
        }
        $dest_channel = $this->request->get('dest_channel');
        if ($dest_channel === null) {
            return $this->error('`dest_channel` is required');
        }
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[账号转移] 服务器: $server_id 玩家原区号: $old_server_id 新uid: $uid ".
                              "旧uid: $old_uid 当前渠道号: $src_channel 新渠道号: $dest_channel");

        return $this->success($ret);
    }

	public function silencePlayerAction()
    {
        $this->view->disable();
        ...
        $silence_hours = $this->request->get('silence_hours');
        if ($silence_hours === null) {
            return $this->error('`silence_hours` is required');
        }
        ...
        $server_desc = $server_model->desc;
        if ($silence_hours == 0) {
            $this->opLog("[解禁玩家] 服务器: $server_desc 玩家ID: $player_id");
        } else {
            $this->opLog("[禁言玩家] 服务器: $server_desc 玩家ID: $player_id ".
                         "禁停时间: ${silence_hours}小时");
        }
    ...
    public function doGmLoginAction()
    {
        ...
        $server_desc = $server_model->desc;
        if ($login_switch == 0) {
            $this->opLog("[GM登录设置关闭] 服务器: $server_desc 玩家ID: $player_id");
        } else {
            $this->opLog("[GM登录设置开启] 服务器: $server_desc 玩家ID: $player_id");
        }
    ...
    public function doGmPayAction()
    {
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[GM充值] 服务器: $server_desc ".
                     "玩家ID: $player_id 金额: ${amount}元");
    ...
    public function doGmConsumeDiamondAction()
    {
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[GM扣元宝] 服务器: $server_desc ".
                     "玩家ID: $player_id 金额: ${amount}元");

        return $this->success($ret);
    }

    public function doGmRenameAction()
    {
        $this->view->disable();
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[GM玩家强制改名] 服务器: $server_desc ".
                     "玩家ID: $player_id 玩家修改名: ${name}");
    ...
    public function doGmFinishCopyAction()
    {
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[GM完成副本] 服务器: $server_desc ".
                     "玩家ID: $player_id 副本ID: $copy_id");
    ...
    public function doGmFinishEliteCopyAction()
    {
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[GM完成精英副本] 服务器: $server_desc ".
                     "玩家ID: $player_id 副本ID: $copy_id");
    ...
    public function doOnekeyDevelopAction()
    {
        ...
        // op log
        $server_desc = $server_model->desc;
        $this->opLog("[一键培养] 服务器: $server_desc ".
                     "玩家ID: $player_id 培养ID: $develop_id");
